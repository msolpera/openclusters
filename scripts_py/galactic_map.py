...
def main():
    ...
    df = ascii.read("../final_table_goodage_actualizado.dat")
    cg = ascii.read('../cg2020.dat')
    cg['NAME'] = [i.lower() for i in cg['NAME']]
    cg_as = join(df, cg, keys='NAME')
    
    df['DB'] = cg_as['r50']*10

    d_pc = 10**(df['dm_median']/5)*10
    ...
    fig.savefig('galactic_map.pdf', bbox_inches='tight')
